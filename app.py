...
# ─────────────────────────────────────────────
#  SHARED MOCK MARKET PARAMETERS
#  Replace with real forward-looking estimates
# ─────────────────────────────────────────────

ETF_UNIVERSE = ["VTI", "VXUS", "BND", "GLD", "VNQ", "QQQ", "SGOV"]

ANN_MU  = np.array([0.12, 0.10, 0.04, 0.08, 0.08, 0.18, 0.05])
ANN_SIG = np.array([0.15, 0.16, 0.05, 0.14, 0.18, 0.22, 0.005])
CORR    = np.array([
    [1.00, 0.85, 0.10, 0.05, 0.60, 0.90, -0.05],
    [0.85, 1.00, 0.08, 0.03, 0.55, 0.80, -0.04],
    [0.10, 0.08, 1.00, 0.15, 0.20, 0.05,  0.30],
    [0.05, 0.03, 0.15, 1.00, 0.10, 0.02, -0.02],
    [0.60, 0.55, 0.20, 0.10, 1.00, 0.65, -0.03],
    [0.90, 0.80, 0.05, 0.02, 0.65, 1.00, -0.06],
    [-0.05,-0.04, 0.30,-0.02,-0.03,-0.06,  1.00],
])
RF_ANN = 0.04
...
def mock_risk_score(form_data):
    ...
    if   score <= 2.0:  vol_ceiling = 0.05
    elif score <= 4.0:  vol_ceiling = 0.08
    elif score <= 6.0:  vol_ceiling = 0.13
    elif score <= 8.0:  vol_ceiling = 0.18
    else:               vol_ceiling = 0.24
    return {"score": score, "vol_ceiling": vol_ceiling}


def mock_portfolio_weights(vol_ceiling):
    """Replace with: optimize(mu, sigma, corr, vol_ceiling)"""
    if   vol_ceiling <= 0.06: w = [0.10, 0.05, 0.55, 0.10, 0.05, 0.00, 0.15]
    elif vol_ceiling <= 0.10: w = [0.25, 0.15, 0.35, 0.10, 0.05, 0.05, 0.05]
    elif vol_ceiling <= 0.15: w = [0.35, 0.20, 0.15, 0.08, 0.08, 0.10, 0.04]
    elif vol_ceiling <= 0.20: w = [0.30, 0.22, 0.05, 0.05, 0.10, 0.25, 0.03]
    else:                     w = [0.20, 0.15, 0.00, 0.05, 0.08, 0.50, 0.02]
    w = np.array(w, dtype=float)
    w /= w.sum()
    return dict(zip(ETF_UNIVERSE, [round(float(x), 4) for x in w]))
...
def run_simulation(weights, horizon_days=252, n_paths=1000, start_value=100_000, seed=42):
    """
...
def compute_mc_stats(paths, daily_rets, start_value, vol_ceiling, horizon_years):
    ...
    path_sharpes  = np.where(path_vol_raw > 0, (path_ret_raw - RF_ANN) / path_vol_raw, np.nan)
...
def generate_gif(paths, start_value, horizon_years):
    ...
    for end in range(1, n_paths + 1, step):
        ...
        ax.set_title(f"Monte Carlo Simulation  |  {end} / 1,000 paths  |  {horizon_years}yr", color=LIGHT, fontsize=11, pad=10)
        ...
        ax.text(0.02, 0.96, stats_text, transform=ax.transAxes,
                color=LIGHT, fontsize=7.5, alpha=0.85,
    ...
    pil_frames[0].save(gif_buf, format="GIF", save_all=True,
                       append_images=pil_frames[1:], loop=0, duration=150)
    gif_buf.seek(0)
...
def build_results(weights, vol_ceiling, start_value=100_000, horizon_years=1):
    ...
    paths, daily_rets = run_simulation(weights, horizon_days=horizon_days, start_value=start_value)
